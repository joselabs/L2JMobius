...
public class UseItem implements ClientPacket
{
	...
	public void run(GameClient client)
	{
		...
		if (item.isEquipable())
		{
			...
			if (item.isArmor() && (item.getArmorItem().getItemType() == ArmorType.SHIELD) && (CategoryData.getInstance().isInCategory(CategoryType.DEATH_KNIGHT_ALL_CLASS, player.getClassId().getId()) || CategoryData.getInstance().isInCategory(CategoryType.SYLPH_ALL_CLASS, player.getClassId().getId()) || CategoryData.getInstance().isInCategory(CategoryType.VANGUARD_ALL_CLASS, player.getClassId().getId()) || CategoryData.getInstance().isInCategory(CategoryType.ASSASSIN_ALL_CLASS, player.getClassId().getId())))
			{
				player.sendPacket(SystemMessageId.YOU_DO_NOT_MEET_THE_REQUIRED_CONDITION_TO_EQUIP_THAT_ITEM);
				return;
			}
