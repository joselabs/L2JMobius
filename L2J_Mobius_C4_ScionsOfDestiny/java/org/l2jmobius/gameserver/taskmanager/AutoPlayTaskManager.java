	...
	private class AutoPlay implements Runnable
	{
		...
		public void run()
		{
			...
			PLAY: for (Player player : _players)
			{
				...
				{
					PICKUP: for (WorldObject wo : player.getKnownList().getKnownObjects().values())
					{
						if ((wo == null) || !wo.isItem() || (Util.calculateDistance(player, wo, true) > 200))
						{
							continue;
						}
						final Item droppedItem = (Item) wo;
						
						// Check if item is reachable.
						if ((!droppedItem.isSpawned()) //
						...
						if (player.calculateDistance2D(droppedItem) > 20)
						{
							if (!player.isMoving())
							{
								player.getAI().setIntention(CtrlIntention.AI_INTENTION_MOVE_TO, new Location(droppedItem.getX(), droppedItem.getY(), droppedItem.getZ()));
							}
							continue PLAY;
						}
						
						// Try to pick it up.
						if (!droppedItem.getDropProtection().isProtected() || (droppedItem.getOwnerId() == player.getObjectId()))
						{
