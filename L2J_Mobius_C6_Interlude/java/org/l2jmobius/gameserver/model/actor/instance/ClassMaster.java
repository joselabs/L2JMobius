...
public class ClassMaster extends Folk
{
	private static ClassMaster _instance;
	...
	/**
	 * Gets the single instance of ClassMaster.
	 * @return single instance of ClassMaster
	 */
	...
	public void onAction(Player player)
	{
		...
		else if (!canInteract(player) && !Config.ALLOW_REMOTE_CLASS_MASTERS) // Calculate the distance between the Player and the Npc
		{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if (command.startsWith("upgrade_hatchling") && Config.CLASS_MASTER_STRIDER_UPDATE)
		{
			...
			for (int i = 0; i < 3; i++)
			{
				final Item collar = player.getInventory().getItemByItemId(hatchCollar[i]);
				if (collar != null)
				{
	 ...
	 */
	private void showChatWindowChooseClass(Player player)
	{
		final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
		final StringBuilder sb = new StringBuilder();
		sb.append("<html>");
	 ...
	 * @param player the player
	 */
	private void showChatWindow1st(Player player)
	{
		final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
		final StringBuilder sb = new StringBuilder();
		sb.append("<html>");
	 ...
	 * @param player the player
	 */
	private void showChatWindow2nd(Player player)
	{
		final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
		final StringBuilder sb = new StringBuilder();
		sb.append("<html>");
	 ...
	 * @param player the player
	 */
	private void showChatWindow3rd(Player player)
	{
		final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
		final StringBuilder sb = new StringBuilder();
		sb.append("<html>");
	 ...
	 * @param player the player
	 */
	private void showChatWindowBase(Player player)
	{
		final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
		final StringBuilder sb = new StringBuilder();
		sb.append("<html>");
	 ...
	 */
	private void changeClass(Player player, int value)
	{
