...
public class UserInfo extends AbstractMaskPacket<UserInfoType>
{
	...
	public void writeImpl(GameClient client, WritableBuffer buffer)
	{
		...
		if (containsMask(UserInfoType.STAT_POINTS)) // 235
		{
			...
			buffer.writeShort(_player.getLevel() < 76 ? 0 : (_player.getLevel() - 75) + _player.getVariables().getInt(PlayerVariables.ELIXIRS_AVAILABLE, 0)); // Usable points
		...
		{
			buffer.writeInt(_player.getVariables().getInt(PlayerVariables.ELIXIRS_AVAILABLE, 0)); // count
			buffer.writeShort(0);
		}
	}
