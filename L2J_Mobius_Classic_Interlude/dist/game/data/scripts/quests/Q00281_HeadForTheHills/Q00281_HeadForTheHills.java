...
public class Q00281_HeadForTheHills extends Quest
{
	...
	public static void giveNewbieReward(Player player)
	{
		...
		if ((player.getLevel() < 25) && !vars.getBoolean("NEWBIE_SHOTS", false))
		{
			...
			{
				giveItems(player, SPIRITSHOTS_NO_GRADE_FOR_ROOKIES.getId(), SPIRITSHOTS_NO_GRADE_FOR_ROOKIES.getCount());
				playSound(player, "tutorial_voice_27");
			}
			else
			{
				giveItems(player, SOULSHOTS_NO_GRADE_FOR_ROOKIES.getId(), SOULSHOTS_NO_GRADE_FOR_ROOKIES.getCount());
