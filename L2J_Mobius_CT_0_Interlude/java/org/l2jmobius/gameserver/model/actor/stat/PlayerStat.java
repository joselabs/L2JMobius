...
public class PlayerStat extends PlayableStat
{
	...
	private void updateVitalityLevel(boolean quiet)
	{
		...
		if (!quiet && (level != _vitalityLevel))
		{
			...
			{
				getActiveChar().sendMessage("Your Vitality has decreased.");
			}
			else
			{
				getActiveChar().sendMessage("Your Vitality has increased.");
			}
			if (level == 0)
			{
				getActiveChar().sendMessage("Your Vitality is fully exhausted.");
			}
			else if (level == 4)
			{
				getActiveChar().sendMessage("Your Vitality is at maximum.");
			}
		}
