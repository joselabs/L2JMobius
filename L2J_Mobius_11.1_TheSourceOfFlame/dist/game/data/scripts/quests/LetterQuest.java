	...
	public void onPlayerBypass(OnPlayerBypass event)
	{
		...
		if (event.getCommand().equals(getTeleportCommand()))
		{
			if ((qs != null) && qs.isCond(1) && hasQuestItems(player, _startSOE))
			{
				...
				{
					showOnScreenMsg(player, NpcStringId.YOU_CANNOT_TELEPORT_DURING_A_SIEGE, ExShowScreenMessage.TOP_CENTER, 5000);
				}
				else if (player.isInParty())
				{
					showOnScreenMsg(player, NpcStringId.YOU_CANNOT_TELEPORT_DURING_A_PARTY_BATTLE, ExShowScreenMessage.TOP_CENTER, 5000);
				}
				else if (player.isInInstance())
				{
					showOnScreenMsg(player, NpcStringId.YOU_CANNOT_TELEPORT_WHILE_FIGHTING_IN_AN_INSTANCE_ZONE, ExShowScreenMessage.TOP_CENTER, 5000);
				}
