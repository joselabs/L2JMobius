...
public class ExAbilityOfFireExpUp extends ServerPacket
{
	...
	public void write()
	{
		ServerPackets.EX_ENHANCED_ABILITY_OF_FIRE_EXP_UP.writeId(this);
		writeByte(_type);
		...
		else if (_type == 1) // Life Source
		{
			...
			writeByte(_success); // int Result
			writeInt(_player.getVariables().getInt(PlayerVariables.CONQUEST_ABILITY_LIFE_SOURCE_EXP, 0)); // int EXP
			writeInt(_player.getVariables().getInt(PlayerVariables.CONQUEST_ABILITY_LIFE_SOURCE_UPGRADES, 500)); // int ExpUpCount
			if (_success == 1)
			{
				// Rewards Part
				if (AbstractScript.getRandom(100) < Config.CONQUEST_ABILITY_LIFE_SOURCE_REWARD_CHANCE)
				{
					...
					writeInt(Config.CONQUEST_ABILITY_LIFE_SOURCE_REWARDS.size()); // array<_ItemInfo> rewards
					for (ItemHolder rewards : Config.CONQUEST_ABILITY_LIFE_SOURCE_REWARDS)
					{
						writeInt(rewards.getId());
						writeLong(rewards.getCount());
					}
				}
				else
				{
					writeInt(0); // array<_ItemInfo> rewards
				}
			}
			else
			{
		...
		else if (_type == 2) // Flame Spark
		{
			...
			writeByte(_success); // int Result
			writeInt(_player.getVariables().getInt(PlayerVariables.CONQUEST_ABILITY_FLAME_SPARK_EXP, 0)); // int EXP
			writeInt(_player.getVariables().getInt(PlayerVariables.CONQUEST_ABILITY_FLAME_SPARK_UPGRADES, 60)); // int ExpUpCount
			if (_success == 1)
			{
				// Rewards Part
				if (AbstractScript.getRandom(100) < Config.CONQUEST_ABILITY_LIFE_SOURCE_REWARD_CHANCE)
				{
					...
					writeInt(Config.CONQUEST_ABILITY_FLAME_SPARK_REWARDS.size()); // array<_ItemInfo> rewards
					for (ItemHolder rewards : Config.CONQUEST_ABILITY_FLAME_SPARK_REWARDS)
					{
						writeInt(rewards.getId());
						writeLong(rewards.getCount());
		...
		else if (_type == 3) // Fire Totem
		{
			...
			writeByte(_success); // int Result
			writeInt(_player.getVariables().getInt(PlayerVariables.CONQUEST_ABILITY_FIRE_TOTEM_EXP, 0)); // int EXP
			writeInt(_player.getVariables().getInt(PlayerVariables.CONQUEST_ABILITY_FIRE_TOTEM_UPGRADES, 100)); // int ExpUpCount
			if (_success == 1)
			{
				// Rewards Part
				if (AbstractScript.getRandom(100) < Config.CONQUEST_ABILITY_FIRE_TOTEM_REWARD_CHANCE)
				{
					...
					writeInt(Config.CONQUEST_ABILITY_FIRE_TOTEM_REWARDS.size()); // array<_ItemInfo> rewards
					for (ItemHolder rewards : Config.CONQUEST_ABILITY_FIRE_TOTEM_REWARDS)
					{
						writeInt(rewards.getId());
						writeLong(rewards.getCount());
		...
		else if (_type == 4) // Battle Soul
		{
			...
			writeByte(_success); // int Result
			writeInt(_player.getVariables().getInt(PlayerVariables.CONQUEST_ABILITY_BATTLE_SOUL_EXP, 0)); // int EXP
			writeInt(_player.getVariables().getInt(PlayerVariables.CONQUEST_ABILITY_BATTLE_SOUL_UPGRADES, 100)); // int ExpUpCount
			if (_success == 1)
			{
				// Rewards Part
				if (AbstractScript.getRandom(100) < Config.CONQUEST_ABILITY_BATTLE_SOUL_REWARD_CHANCE)
				{
					...
					writeInt(Config.CONQUEST_ABILITY_BATTLE_SOUL_REWARDS.size()); // array<_ItemInfo> rewards
					for (ItemHolder rewards : Config.CONQUEST_ABILITY_BATTLE_SOUL_REWARDS)
					{
						writeInt(rewards.getId());
						writeLong(rewards.getCount());
